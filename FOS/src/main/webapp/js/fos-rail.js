...
Fos.ContainerTraceTab = function(p) {
	...
	if(p.get('rowAction')!='N') store.load({params:{consId:p.get('consId')}});
	...
	{header:C_CONT_OWNER,dataIndex:'contOwner',editor:new Ext.form.ComboBox({
	...
	this.addTrace = function(){
		var c = new FContainer({id:GGUID(),contId:'0',consId:p.get('consId'),consNo:p.get('consNo'),
			consEta:p.get('consEta'),version:'0'});
	...
	this.saveTrace = function(){
		...
		if(a.length){
			...
			if(xml!=''){
			 	Ext.Ajax.request({scope:this,url:SERVICE_URL,method:'POST',params:{A:'COTR_S'},
	...
	Fos.ContainerTraceTab.superclass.constructor.call(this, {title:C_CONT_TRACE,
		border:false,autoScroll:true,header:false,deferredRender:false,
...
Fos.SectionGridCustFee = function(p,section,parent,listStore,traceSm) {
	...
	store.load({params:{consId:p.get('consId'),section:section,expeType:'R'}});
	...
	var t1={header:C_SETTLE_OBJECT,width:200,dataIndex:"custName",align:'left',
        ...
        invalidText:'',selectOnFocus:true,listClass:'x-combo-list-small',
        store:getCS(),enableKeyEvents:true,
        listeners:{scope:this,
        	select:function(c,r,i){
			...
			for(var i=0;i<a.length;i++){
				for(var j=0;j<b.length;j++){
					if(a[i].get('cotrId')!=b[j].get('consCustId')){
					    ...
					    if(num>0){
							var e = new SExpense({id:rid,expeId:rid,consId:a[i].get('consId'),consNo:a[i].get('consNo'),
								section:section,consBizClass:a[i].get('consBizClass'),consBizType:a[i].get('consBizType'),
					    		consShipType:a[i].get('consShipType'),expeStatus:'0',expeBillStatus:'0',
					    		expeInvoiceStatus:'0',expeWriteOffStatus:'0',
					    		consCustId:a[i].get('cotrId'),
					    		custId:p.get('custId'),custName:p.get('custName'),
					    		expeType:'R',pateId:PPID,pateCode:'P',unitName:unit,currCode:'CNY',expeUnitPrice:a[i].get('custExtendedRate'),
					    		...
					    		expeTotalAmount:expeTotalAmount,expeNum:num,version:'0'});
					    	this.stopEditing();store.add(e);e.set('rowAction','N');sm.selectFirstRow();this.startEditing(0, 1);
					    }
			        }
				}
		     }
	...
	var b1={itemId:'TB_A',text:C_RECALCULATE,iconCls:'dollar',scope:this,handler:this.add};
	var b2={itemId:'TB_B',text:C_REMOVE,iconCls:'remove',scope:this,handler:this.removeExp};
	var b3={itemId:'TB_C',text:C_SAVE,iconCls:'save',scope:this,handler:this.save};
	
	Fos.SectionGridCustFee.superclass.constructor.call(this, {title:C_CUST_FEE,
	border:true,autoScroll:true,clicksToEdit:1,height:160,
...
Fos.SectionGridFee = function(p,section,parent,listStore,traceSm) {
	var store = GS('EXPE_PERM_Q','SExpense',SExpense,'expeId','DESC','','','',false);
	store.load({params:{consId:p.get('consId'),section:section,expeType:'P'}});
	var t1={header:C_SETTLE_OBJECT,width:200,dataIndex:"custName",align:'left',
		editor:new Ext.form.ComboBox({displayField:'custCode',valueField:'custNameCn',triggerAction:'all',
        mode:'local',tpl:custTpl,itemSelector:'div.list-item',listWidth:400,allowBlank:false,blankText:'',
        invalidText:'',selectOnFocus:true,listClass:'x-combo-list-small',
					...
					if(a[i].get('cotrId')==b[j].get('consCustId')){
					    ...
					    if(num>0){
							var e = new SExpense({id:rid,expeId:rid,consId:a[i].get('consId'),consNo:a[i].get('consNo'),
					    		...
					    		expeInvoiceStatus:'0',expeWriteOffStatus:'0',
					    		consCustId:a[i].get('cotrId'),
					    		custId:a[i].get('contOwnerId'),custName:a[i].get('contOwner'),
					    		expeType:'P',pateId:PPID,pateCode:'P',unitName:unit,currCode:'CNY',expeUnitPrice:a[i].get('tenantExtendedRate'),
					    		expeCommissionRate:'',expeCommission:'',expeInnerPrice:'',expeExRate:'1.0000',expeRemarks:'',expeDate:new Date(),
					    		expeAllocationFlag:'0',expeAllocatedFlag:'0',
					    		expeInvoiceAmount:'0',expeWriteOffAmount:'0',expeWriteOffRcAmount:'0',expeInnerAmount:'0',expeRcAmount:'0',
					    		expeTotalAmount:expeTotalAmount,expeNum:num,version:'0'});
	...
	Fos.SectionGridFee.superclass.constructor.call(this, {title:C_OWNER_FEE,
	border:true,autoScroll:true,clicksToEdit:1,height:160,
    stripeRows:true,store:store,sm:sm,cm:cm,listeners:{scope:this,
		beforeedit:function(e){
			e.cancel = e.record.get('editable')==0||e.record.get('expeStatus')>0||e.record.get('expeInvoiceStatus')>0||e.record.get('expeWriteOffStatus')>0;
...
Fos.RailwayGrid = function(bizClass,bizType){
    ...
    var cm=new Ext.grid.ColumnModel({columns:[
        ...
        {header:C_CONT_NO,dataIndex:"consContainerNo"},
        {header:C_CRAIL_NO,dataIndex:"vessName"},
        {header:C_DEPARTURE_DATE,width:80,dataIndex:"consSailDate",renderer:formatDate},
        {header:C_DEPARTURE_STATION,dataIndex:"consPolCn"},
        {header:C_CTRANSFER_STATION,width:80,dataIndex:"consPotCn"},
        {header:C_CDELIVERY_DATE,dataIndex:"consDeliveryDate",renderer:formatDate},
        {header:C_FTRANSFER_STATION,width:80,dataIndex:"consPodEn"},
        {header:C_FDELIVERY_DATE,dataIndex:"consLoadDate",renderer:formatDate},
        {header:C_FOREIGN_RAIL_NO,width:80,dataIndex:"voyaName"},
        {header:C_DESTINATION_STATION,width:80,dataIndex:"consDeliveryPlace"},
        {header:C_ARRIVAL_DATE,dataIndex:"consEta",renderer:formatDate}
        ],
	...
	var b4={text:C_EXPORT,disabled:NR(m+F_E),iconCls:'print',scope:this,menu: 
		{items: [{text:C_CUSTOMS_STAT,scope:this,handler:this.exp1}]}};	
	var b5={text:C_FAST_SEARCH,iconCls:'search',handler:this.fastSearch};	
	var b6={text:C_RESET,iconCls:'refresh',handler:this.reset};
	var b7={text:C_SEARCH,disabled:NR(m+F_V),iconCls:'search',handler:this.search};		
...
Fos.RailwayConsign = function (p,listStore) {
    ...
    //单票tbar
	var b1={text:C_SAVE+'(S)',itemId:'TB_S',iconCls:'save',disabled:NR(m+F_M)||locked||disable,scope:this,handler:this.save};
	
	var tbs=[b1];
	
	//单票基本信息
	var railNo = {fieldLabel:C_CONS_NO,style:'{font-weight:bold;color:green;}',disabled:true,tabIndex:1,
			name:'consNo',value:p.get('consNo'),xtype:'textfield',anchor:'99%'};
	//委托单位
	var custName = {fieldLabel:C_BOOKER,itemCls:'required',tabIndex:5,name:'custName',
			...
			selectOnFocus:true,anchor:'99%',
		  	...
		  	keydown:{fn:function(f,e){LC(f,e,'custBookerFlag');},buffer:BF}}};
	//委托日期
	var consDate =  {fieldLabel:C_CONS_DATE,tabIndex:13,name:'consDate',value:p.get('consDate'),
	  		xtype:'datefield',format:DATEF,anchor:'99%'};
	//操作员
	var operator = {fieldLabel:C_OPERATOR,itemCls:'required',tabIndex:2,name:'consOperatorId',value:p.get('consOperatorId'),
	...
	//客户联系人
	var custContact ={fieldLabel:C_CONTACT,tabIndex:6,name:'custContact',value:p.get('custContact'),xtype:'textfield',anchor:'99%'};
	//业务员
	var consSalesRepName = {fieldLabel:C_SALES,itemCls:'required',tabIndex:3,name:'consSalesRepName',value:p.get('consSalesRepName'),
	...
	//客户联系人电话
	var custTel = {fieldLabel:C_PHONE,tabIndex:7,name:'custTel',value:p.get('custTel'),
					xtype:'textfield',anchor:'99%'};
	//客户业务号
	var grouName = {fieldLabel:C_DEPT,itemCls:'required',tabIndex:4,name:'deptId',value:p.get('deptId'),
	...
	//客户传真
    var custFax = {fieldLabel:C_FAX,xtype:'textfield',tabIndex:8,name:'custFax',value:p.get('custFax'),anchor:'99%'};     
	
    //客户业务员
    var custSales={fieldLabel:C_CUST_SALES,name:'custSalesName',value:p.get('custSalesName'),
    ...
    //客户业务号
    var refNo={fieldLabel:C_REF_NO,tabIndex:11,name:'consRefNo',value:p.get('consRefNo'),xtype:'textfield',anchor:'99%'};    
    //合同号
    var contractNo={fieldLabel:C_CONTRACT_NO,tabIndex:12,name:'consContractNo',
			value:p.get('consContractNo'),xtype:'textfield',anchor:'99%'};
    //运输条款
    var tranTerm={fieldLabel:C_TTER,itemCls:'needed',tabIndex:17,name:'tranId',value:p.get('tranId'),store:p.get('consBizType')==BT_B?getTTB_S():getTTC_S(),xtype:'combo',displayField:'tranCode',valueField:'tranId',typeAhead: true,mode: 'local',triggerAction: 'all',selectOnFocus:true,anchor:'99%',
    		listeners:{scope:this,
    		blur:function(f){if(f.getRawValue()==''){f.clearValue();p.set('tranId','');}},
        	select:function(c,r,i){p.set('tranCode',r.get('tranCode'));}}};
    //运费条款
    var tranPate={fieldLabel:C_PATE,itemCls:'needed',tabIndex:15,name:'pateId',value:p.get('pateId'),store:getPATE_S(),xtype:'combo',displayField:'pateName',valueField:'pateId',typeAhead: true,mode: 'local',triggerAction: 'all',selectOnFocus:true,anchor:'99%',
        	listeners:{scope:this,
    ...
    //提单信息
    //发车日期
    var departureDate = {fieldLabel:C_DEPARTURE_DATE,tabIndex:14,name:'consSailDate',value:p.get('consSailDate'),
			xtype:'datefield',format:DATEF,anchor:'99%'};
    //始发站
    var departureStation={fieldLabel:C_DEPARTURE_STATION,tabIndex:39,
    		name:'consPolCn',value:p.get('consPolCn'),store:getTS(),xtype:'combo',
    		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
    		triggerAction:'all',selectOnFocus:true,anchor:'99%',
    		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
    		listeners:{scope:this,
            	...
            	select:function(c,r,i){
             	...
             	keydown:{fn:LT,buffer:BF}}};
    //中方换装站
    var cTransferStation={fieldLabel:C_CTRANSFER_STATION,tabIndex:39,
    		name:'consPotCn',value:p.get('consPotCn'),store:getTS(),xtype:'combo',
    		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
    		triggerAction:'all',selectOnFocus:true,anchor:'99%',
    		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
    		listeners:{scope:this,
            	...
            	select:function(c,r,i){
             	...
             	keydown:{fn:LT,buffer:BF}}};
    //中方车号
    var cRailNo = {fieldLabel:C_CRAIL_NO,tabIndex:14,name:'vessName',value:p.get('vessName'),
			xtype:'textfield',format:DATEF,anchor:'99%'};
    //中方交出时间
    var cDeliveryDate = {fieldLabel:C_CDELIVERY_DATE,tabIndex:14,name:'consDeliveryDate',value:p.get('consDeliveryDate'),
			xtype:'datefield',format:DATEF,anchor:'99%'};
    //外方换装站
    var fTransferStation={fieldLabel:C_FTRANSFER_STATION,tabIndex:39,
    		name:'consPodEn',value:p.get('consPodEn'),store:getTS(),xtype:'combo',
    		displayField:'trainNameEn',valueField:'trainNameEn',typeAhead: true,mode:'local',
    		triggerAction:'all',selectOnFocus:true,anchor:'99%',
    		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
    		listeners:{scope:this,
    			blur:function(f){if(f.getRawValue()==''){f.clearValue();p.set('consPol','');}},
            	select:function(c,r,i){
             	...
             	keydown:{fn:LT,buffer:BF}}};
    //外方车号
    var fRailNo = {fieldLabel:C_FRAIL_NO,tabIndex:14,name:'voyaName',value:p.get('voyaName'),
			xtype:'textfield',format:DATEF,anchor:'99%'};
    //外方交出时间
    var fDeliveryDate = {fieldLabel:C_FDELIVERY_DATE,tabIndex:14,name:'consLoadDate',value:p.get('consLoadDate'),
			xtype:'datefield',format:DATEF,anchor:'99%'};
    //到站
    var destinationStation={fieldLabel:C_DESTINATION_STATION,tabIndex:39,
    		name:'consDeliveryPlace',value:p.get('consDeliveryPlace'),store:getTS(),xtype:'combo',
    		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
    		triggerAction:'all',selectOnFocus:true,anchor:'99%',
    		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
    		listeners:{scope:this,
            	...
            	select:function(c,r,i){
             	...
             	keydown:{fn:LT,buffer:BF}}};
  //到站日期
    var arrivalDate = {fieldLabel:C_ARRIVAL_DATE,tabIndex:14,name:'consEta',value:p.get('consEta'),
			xtype:'datefield',format:DATEF,anchor:'99%'};
    //箱号
    var containerNo = {fieldLabel:C_CONT_NO,name:'consContainerNo',value:p.get('consContainerNo'),tabIndex:40,xtype:'textfield',
    	anchor:'99%'};
    //箱类型：0：货主SOC；1：我司SOC；2：COC
    //用consContainerCompany箱公司代替
    var containerType = {fieldLabel:C_CONTAINER_TYPE,name:'consContainerCompany',value:p.get('consContainerCompany'),
    		xtype:'combo',store:CONT_TYPE_S,displayField:'NAME',valueField:'CODE',typeAhead: true,mode:'local',
    		triggerAction:'all',selectOnFocus:true,anchor:'99%'};
  //用consWarehouseNo 代替 铅封号
    var contSealNo = {fieldLabel:C_SEAL_NO,name:'consWarehouseNo',value:p.get('consWarehouseNo'),tabIndex:40,xtype:'textfield',
    	anchor:'99%'};
	...
	var t11={layout:'column',title:C_CONS_INFO,collapsible:true,border:false,padding:5,
	    	items:[{columnWidth:.25,layout:'form',border:false,labelWidth:70,items:t111},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:70,items:t112},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:70,items:t113},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:70,items:t114}
			]};
	var t12={layout:'column',title:C_BL_INFO,collapsible:true,border:false,padding:5,
			items:[{columnWidth:.25,layout:'form',border:false,labelWidth:80,items:t121},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:80,items:t122},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:80,items:t123},
	    	       {columnWidth:.25,layout:'form',border:false,labelWidth:80,items:t124}
			]};
	var t13={layout:'column',title:C_SHIPPER_INFO,collapsible:true,padding:5,border:false,items:
	...
	var t1={id:'C_R_J_'+p.get('id'),title:C_BASE_INFO+'(J)',border:false,height:400,
			items:[t11,t12,t13]};
	//单票基本信息--end
	//单票货物信息
...
var showG_STATION = function() {
    ...
    var c2={header:C_CODE,dataIndex: 'trainCode',editor:new Ext.form.TextField({allowBlank:false,blankText:'',invalidText:''})};
    var c3={header:C_ENAME,dataIndex: 'trainNameEn',editor:new Ext.form.TextField({allowBlank:false,blankText:'',invalidText:''})};
    var c4={header:C_CNAME,dataIndex: 'trainNameCn',editor:new Ext.form.TextField({allowBlank:true,blankText:'',invalidText:''})};
	var cm=new Ext.grid.ColumnModel({columns:[sm,c1,c2,c3,c4,ac],defaults:{sortable:true,width:100}});
	var ts=new Ext.data.SimpleStore({id:0,fields:['CODE','NAME'],data:[['trainCode',C_CODE],['trainNameEn',C_ENAME],['trainNameCn',C_CNAME],['counCode',C_COUN_CODE]]});
	var st = new Ext.form.ComboBox({width:80,store:ts,value:'trainNameEn',displayField:'NAME',valueField:'CODE',typeAhead:true,mode:'local',forceSelection: true,triggerAction:'all',selectOnFocus:true});
    ...
    id:'G_STATION',iconCls:'gen',title:C_STATION,header:false,plugins:ac,clicksToEdit:1,closable:true,	
    store: store,sm:sm,cm:cm,loadMask:true,
	view: new Ext.grid.GroupingView(groupViewCfg),
	bbar:PTB(store,100),
	tbar:[{
		text:C_ADD,iconCls:'add',handler : function(){
			var p = new GTrainStation({id:GGUID(),trainId:'0',trainType:'0',active:1,version:'0',rowAction:'N'});
        	grid.stopEditing();store.insert(0,p);grid.startEditing(0, 1);}},'-',
        {text:C_REMOVE,iconCls:'remove',handler:function(){FOS_REMOVE(sm,store);}},'-', 
        {text:C_SAVE,iconCls:'save',handler:function(){FOS_POST(store,'GTrainStation',GTrainStation,'TRAIN_S');getTRAIN_S().reload();}},'-',
        {text:C_EXPORT,iconCls:'print',handler:function(){EXP('C','PORT','');}},'-', 
...
Fos.RailConsLookupWin = function(store,setQueryParams){    
	this.reload=function(){
     	var a=[];var op=1;
     	a[a.length]=new QParam({key:'consBizType',value:BT_T,op:1});
     	//委托单位ID
 		...
 		if(consContainerNo) 
 			a[a.length]=new QParam({key:'consContainerNo',value:consContainerNo,op:op}); 
 		//中方车号
 		...
 		if(consSailDate && consSailDate2){
 			...
 			a[a.length]=new QParam({key:'consSailDate',value:consSailDate.format(DATEF),op:op});
 		//中方换装站
 		var consPotCn=panel.find('name','consPotCn')[0].getValue();
 		if(consPotCn) 
 			a[a.length]=new QParam({key:'consPotCn',value:consPotCn,op:op}); 
 		//中方交出日期
 		...
 		if(consDeliveryDate && consDeliveryDate2){
 			...
 			a[a.length]=new QParam({key:'consDeliveryDate2',value:consDeliveryDate2.format(DATEF),op:3});
 		}
 		...
 		if(consEta && consEta2){
 			...
 			a[a.length]=new QParam({key:'consEta2',value:consEta2.format(DATEF),op:3});
 		}
 		else if(consEta) 
 			a[a.length]=new QParam({key:'consEta',value:consEta.format(DATEF),op:op});
 		//外方换装站
 		var consPodEn=panel.find('name','consPodEn')[0].getValue();
 		if(consPodEn) 
 			a[a.length]=new QParam({key:'consPodEn',value:consPodEn,op:op}); 
 		//外方换装日期
 		...
 		if(consLoadDate && consLoadDate2){
 			...
 			a[a.length]=new QParam({key:'consLoadDate2',value:consLoadDate2.format(DATEF),op:3});
 		}
 		...
 		if(consPolCn) 
 			a[a.length]=new QParam({key:'consPolCn',value:consPolCn,op:op}); 
 		//目的站
	...
	var panel = new Ext.Panel({plain:true,height:340,layout:'column',
		defaults:{bodyStyle:'padding:10px'},items:
	    	...
	    	items:[{fieldLabel:C_BOOKER,name:'custId',store:getCS(),
        		...
        		triggerAction:'all',selectOnFocus:true,anchor:'90%',
              	...
              	{fieldLabel:C_CRAIL_NO,name:'vessName',xtype:'textfield',anchor:'90%'},
              	{fieldLabel:C_CTRANSFER_STATION,name:'consPotCn',store:getTS(),xtype:'combo',
            		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
            		triggerAction:'all',selectOnFocus:true,anchor:'90%',
            		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
            		listeners:{scope:this,
                     	keydown:{fn:LT,buffer:BF}}},
              	{fieldLabel:C_FRAIL_NO,name:'voyaName',xtype:'textfield',anchor:'90%'},
              	{fieldLabel:C_FTRANSFER_STATION,name:'consPodEn',store:getTS(),xtype:'combo',
            		displayField:'trainNameEn',valueField:'trainNameEn',typeAhead: true,mode:'local',
            		triggerAction:'all',selectOnFocus:true,anchor:'90%',
            		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
            		listeners:{scope:this,
                     	keydown:{fn:LT,buffer:BF}}},
              	{fieldLabel:C_DEPARTURE_STATION,name:'consPolCn',store:getTS(),xtype:'combo',
            		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
            		triggerAction:'all',selectOnFocus:true,anchor:'90%',
            		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
            		listeners:{scope:this,
      	...
      	{columnWidth:.33,layout:'form',border:false,labelWidth:80,labelAlign:"right",
   		items:[{fieldLabel:C_CONS_DATE,name:'consDate',xtype:'datefield',format:DATEF,anchor:'90%'},
		   		{fieldLabel:C_SALES,name:'consSalesRepId',store:getSALE_S(),xtype:'combo',
		          	displayField:'userLoginName',valueField:'userId',typeAhead: true,
		          	mode: 'local',triggerAction: 'all',selectOnFocus:true,anchor:'90%'},
	          	{fieldLabel:C_DEPARTURE_DATE,name:'consSailDate',xtype:'datefield',format:DATEF,anchor:'90%'},
	          	{fieldLabel:C_CDELIVERY_DATE,name:'consDeliveryDate',xtype:'datefield',format:DATEF,anchor:'90%'},
	          	{fieldLabel:C_ARRIVAL_DATE,name:'consEta',xtype:'datefield',format:DATEF,anchor:'90%'},
	          	{fieldLabel:C_FDELIVERY_DATE,name:'consLoadDate',xtype:'datefield',format:DATEF,anchor:'90%'},
	          	{fieldLabel:C_DESTINATION_STATION,name:'consDeliveryPlace',store:getTS(),xtype:'combo',
            		displayField:'trainNameCn',valueField:'trainNameCn',typeAhead: true,mode:'local',
            		triggerAction:'all',selectOnFocus:true,anchor:'90%',
            		tpl:stationTpl,itemSelector:'div.list-item',listWidth:C_LW,enableKeyEvents:true,
            		listeners:{scope:this,
		...
		items:[{fieldLabel:C_TO,name:'consDate2',xtype:'datefield',format:DATEF,anchor:'90%'},
		       {fieldLabel:C_CONT_NO,name:'consContainerNo',xtype:'textfield',anchor:'90%'},
		       {fieldLabel:C_TO,name:'consSailDate2',xtype:'datefield',format:DATEF,anchor:'90%'},
		       {fieldLabel:C_TO,name:'consDeliveryDate2',xtype:'datefield',format:DATEF,anchor:'90%'},
		       {fieldLabel:C_TO,name:'consEta2',xtype:'datefield',format:DATEF,anchor:'90%'},
		       {fieldLabel:C_TO,name:'consLoadDate2',xtype:'datefield',format:DATEF,anchor:'90%'},
		       {fieldLabel:C_CONTAINER_TYPE,name:'consContainerCompany',xtype:'combo',store:CONT_TYPE_S,
		    	   displayField:'NAME',valueField:'CODE',typeAhead: true,mode:'local',
		    		triggerAction:'all',selectOnFocus:true,anchor:'90%'}
		       ]}
        	
	]});
    Fos.RailConsLookupWin.superclass.constructor.call(this, {title:C_CONS_QUERY,iconCls:'search',modal:true,
