...
var checkPassEx=function(){
	...
	if(bP){
		...
		var msg=t.apply([ed.format('Y-m-d')]);
		XMG.alert(SYS,msg);
	}
};
...
Fos.FieldSet = Ext.extend(Ext.form.FieldSet, {
    initComponent: function() {
        Fos.FieldSet.superclass.initComponent.call(this);
        if (this.checkboxToggle && this.checkboxName) {
            this.hiddenCheckboxValue = new Fos.FieldSetHiddenCheckboxValue({name: this.checkboxName});
            this.hiddenCheckboxValue.on('valuechange', function(field, value) {
                if (value=='0' || !value) {this.collapse();} else {this.expand();}}, this);
            this.add(this.hiddenCheckboxValue);
            delete this.checkboxName;
        }
    },
    onCheckClick : function(){this.hiddenCheckboxValue.setValue(this.checkbox.dom.checked?'1':'0');}
});

Fos.FieldSetHiddenCheckboxValue = Ext.extend(Ext.form.Hidden, {
    initComponent: function() {
        this.addEvents({'valuechange': true});
        Fos.FieldSetHiddenCheckboxValue.superclass.initComponent.call(this);
    },
    setValue : function(v) {
        Fos.FieldSetHiddenCheckboxValue.superclass.setValue.call(this, v);
        this.fireEvent('valuechange', this, v);
    }
});
Ext.reg('fieldset', Fos.FieldSet);
...
function N2EW(input){
	...
	for (i = 0; i < inputlength; i++) {
		...
		if (store[9] != "") { a1 ="HUNDRED AND "} else {a1 = ""};
...
Ext.override(Ext.grid.RowSelectionModel, {  
	onEditorKey : function(field, e) {
		...
		else if (k == e.ENTER||e.TAB) {
			e.stopEvent(); ed.completeEdit();  
			if (shift) {newCell = g.walkCells(ed.row, ed.col - 1, -1,this.acceptsNav, this);}
			else {newCell = g.walkCells(ed.row, ed.col + 1, 1, this.acceptsNav, this);}  
		}		
		else if (k == e.ESC) {ed.cancelEdit();}
...
Ext.extend(Ext.ux.MenuPanel, Ext.Panel, {
     afterRender: function(){
        ...
        el.setWidth('98%');
        el.applyStyles({border:'0px',background:'transparent'});
        el.show();
    },	
	getMenu: function(){return this.menu;},
	beforeDestroy: function(){Ext.ux.MenuPanel.superclass.beforeDestroy.call(this);this.menu.destroy();}
 }); 
