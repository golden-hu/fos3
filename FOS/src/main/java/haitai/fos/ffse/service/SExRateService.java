...
public class SExRateService {
	...
	public List<SExRate> query(Map queryMap) {
		...
		for (SExRate exRate : hasList) {
			rateMap.put(exRate.getExraBaseCurrency() + ConstUtil.STRING_SHARP
					+ exRate.getExraExCurrency(), exRate);
		}
		...
		for (GCurrency currency : currList) {
			if(!ConstUtil.CURRENCY_BASE_DEFAULT.equals(currency.getCurrCode())){
				currArray[i][0] = currency.getCurrCode() + ConstUtil.STRING_SHARP
						+ ConstUtil.CURRENCY_BASE_DEFAULT;
				currArray[i][1] = currency.getCurrCode();
				currArray[i][2] = ConstUtil.CURRENCY_BASE_DEFAULT;
				currArray[i + 1][0] = ConstUtil.CURRENCY_BASE_DEFAULT
						+ ConstUtil.STRING_SHARP + currency.getCurrCode();
		...
		for (int j = 0; j < currArray.length; j++) {
			...
			if(!rateMap.containsKey(key)){
				...
				rate.setCompCode(SessionManager
	...
	public ISExRateDAO getDao() {
		return dao;
	}

	@Autowired
	public void setDao(ISExRateDAO dao) {
		this.dao = dao;
	}
}
