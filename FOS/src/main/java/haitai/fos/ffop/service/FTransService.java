...
public class FTransService {
	private IFTransDAO dao = null;
	private IFTransTaskDAO taskDao = null;
	private IFTransCargoDAO cargoDao = null;
		...
		for (Iterator iter = entityList.iterator(); iter.hasNext();) {
			...
			if (obj instanceof FTransTask) {
				...
				if (ConstUtil.ROW_N.equalsIgnoreCase(entity
					...
					//前台传的id(负数)->后台生成id
					entity.setTranId(NumberUtil.frontId2DbId(idMap, entity
							.getTranId()));
					taskDao.save(entity);
					retList.add(entity);
				} else if (ConstUtil.ROW_M.equalsIgnoreCase(entity
	...
	public IFTransDAO getDao() {
		return dao;
	}

	@Autowired
	public void setDao(IFTransDAO dao) {
		this.dao = dao;
	}

	public IFTransTaskDAO getTaskDao() {
		return taskDao;
	}

	@Autowired
	public void setTaskDao(IFTransTaskDAO taskDao) {
		this.taskDao = taskDao;
	}

	public IFTransCargoDAO getCargoDao() {
		return cargoDao;
	}

	@Autowired
	public void setCargoDao(IFTransCargoDAO cargoDao) {
