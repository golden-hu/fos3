...
public class TExportHistoryService {
	...
	private ITExportHistoryDAO dao = null;
	private ITInvoiceDAO expInvoDao = null;
	private ITInvoiceItemDAO expInitDao = null;
	private ISInvoiceDAO realInvoDao = null;
	private ISInvoiceItemDAO realInitDao = null;
	private ITVoucherDAO expVoucDao = null;
	private ITVoucherItemDAO expVoitDao = null;
	private ISVoucherDAO realVoucDao = null;
	private ISVoucherItemDAO realVoitDao = null;
	...
	public TExportHistory copy2Middle(Map<String, Object> queryMap,
		...
		if (type == ConstUtil.HIST_TYPE_AR_INVOICE
				|| type == ConstUtil.HIST_TYPE_AP_INVOICE) {
	...
	public void dump(Map<String, Object> queryMap) {
		...
		tempFileName += ConstUtil.STRING_UNDERLINE + history.getExhiId()
				+ ConstUtil.STRING_UNDERLINE
				+ StringUtil.date2String(TimeUtil.getNow())
				+ ConstUtil.STRING_DOT + ConstUtil.TEMP_SUFFIX_EXCEL;
		try {
			String dir = ConfigUtil.getRealExportDir();
			FileUtil.createDirs(dir);
			FileOutputStream f = new FileOutputStream(ConfigUtil
					.getRealExportDir()
	 ...
	 * @return
	 */
	private Workbook writeArInvoice(List<TInvoice> invoiceList,
			List<TInvoiceItem> invoiceItemList) {
		...
		for (TInvoice item : invoiceList) {
			rowData = new Object[] { item.getExhiId(), item.getInvoId(),
					item.getCustId(), item.getCurrCode(),
					item.getInvoAmount(), "0", item.getInvoBlNo() };
			rowList.add(rowData);
		}
	 ...
	 * @return
	 */
	private Workbook writeApInvoice(List<TInvoice> invoiceList,
			List<TInvoiceItem> invoiceItemList) {
		Workbook wb = new HSSFWorkbook();
		Sheet sheet = wb.createSheet("Invoices");
		List<Object[]> rowList = new ArrayList<Object[]>();
	 ...
	 * @return
	 */
	private Workbook writeApReceipt(List<TVoucher> voucList,
			List<TVoucherItem> voitList) {
		Workbook wb = new HSSFWorkbook();
		...
		for (TVoucher item : voucList) {
			if (item.getVoucPaymentType() == null
					|| 6 == item.getVoucPaymentType()) {
				isCash = true;
			}
	 ...
	 * @return
	 */
	private Workbook writeApPayment(List<TVoucher> voucList,
			List<TVoucherItem> voitList) {
		Workbook wb = new HSSFWorkbook();
	...
	public ITExportHistoryDAO getDao() {
		return dao;
	}

	@Autowired
	public void setDao(ITExportHistoryDAO dao) {
		this.dao = dao;
	}

	public ITInvoiceDAO getExpInvoDao() {
		return expInvoDao;
	}

	@Autowired
	public void setExpInvoDao(ITInvoiceDAO expInvoDao) {
		this.expInvoDao = expInvoDao;
	}

	public ITInvoiceItemDAO getExpInitDao() {
		return expInitDao;
	}

	@Autowired
	public void setExpInitDao(ITInvoiceItemDAO expInitDao) {
		this.expInitDao = expInitDao;
	}

	public ISInvoiceDAO getRealInvoDao() {
		return realInvoDao;
	}

	@Autowired
	public void setRealInvoDao(ISInvoiceDAO realInvoDao) {
		this.realInvoDao = realInvoDao;
	}

	public ISInvoiceItemDAO getRealInitDao() {
		return realInitDao;
	}

	@Autowired
	public void setRealInitDao(ISInvoiceItemDAO realInitDao) {
		this.realInitDao = realInitDao;
	}

	public ITVoucherDAO getExpVoucDao() {
		return expVoucDao;
	}

	@Autowired
	public void setExpVoucDao(ITVoucherDAO expVoucDao) {
		this.expVoucDao = expVoucDao;
	}

	public ITVoucherItemDAO getExpVoitDao() {
		return expVoitDao;
	}

	@Autowired
	public void setExpVoitDao(ITVoucherItemDAO expVoitDao) {
		this.expVoitDao = expVoitDao;
	}

	public ISVoucherDAO getRealVoucDao() {
		return realVoucDao;
	}

	@Autowired
	public void setRealVoucDao(ISVoucherDAO realVoucDao) {
		this.realVoucDao = realVoucDao;
	}

	public ISVoucherItemDAO getRealVoitDao() {
		return realVoitDao;
	}

	@Autowired
	public void setRealVoitDao(ISVoucherItemDAO realVoitDao) {
		this.realVoitDao = realVoitDao;
	}
