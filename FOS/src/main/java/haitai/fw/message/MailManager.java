...
public class MailManager {
	...
	public void sendEmail(PMessage message) {
		...
		logger.info("message id = " + message.getMessId()
				+ ", send flag update success!");
	}
	...
	public void send(PMessage pmsg) {
		...
		try {
			...
			helper.setText(pmsg.getMessBody(),
					pmsg.getMessType() == ConstUtil.MESS_TYPE_FAX ? false
							: true);
			if (StringUtil.isNotBlank(pmsg.getMessAttachment())) {
				...
				fileName = ConfigUtil.getRealTempDir()
