...
public class MethodUtil {
	...
	private static Set<String> getMethodsName(Class<? extends Object> clazz) {
	...
	@SuppressWarnings("unchecked")
	public static void doSetMethod(Object entity, String fieldName,
		...
		if (methodSet.contains("set" + StringUtil.capitalize(fieldName))) {
			try {
				Method method = entity.getClass().getMethod(
						"set" + StringUtil.capitalize(fieldName),
		...
		if (methodSet.contains("get" + StringUtil.capitalize(fieldName))
			...
			try {
				method = entity.getClass().getMethod(
						"get" + StringUtil.capitalize(fieldName));
		...
		if (methodMap == null) {
			...
			for (Method method : methods) {
				if (method.getName().startsWith("set")) {
					methodMap.put(StringUtil.uncapitalize(method.getName()
							.substring(3)), method);
				}
			}
