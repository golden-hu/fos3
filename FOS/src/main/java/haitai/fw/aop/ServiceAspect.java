...
public class ServiceAspect {
	...
	ActionLogUtil actLogUtil = null;
	PTableInfoService tableInfoService = null;
	
	@SuppressWarnings("unused")
	@Pointcut("execution(* haitai.fos.*.service.*Service.query*(..)) "
			+ "&& args(queryMap)")
	private void entityQuery(Map<String, Object> queryMap) {
	}

	@SuppressWarnings("unused")
	@Pointcut("execution(* haitai.fos.*.service.*Service.complexQuery*(..)) "
			+ "&& args(conditions, queryMap)")
	...
	public void addQueryParam(List<FosQuery> conditions,
			Map<String, Object> queryMap) {
		if(conditions == null) {
	...
	@SuppressWarnings("unused")
	@Pointcut("execution(* haitai.fos.*.service.*Service.save*(..)) "
			+ "&& args(entityList)")
	...
	public Object aroundSave(ProceedingJoinPoint jp, List<Object> entityList)
			throws Throwable {
			...
			if (retObj instanceof List) {
				...
				for (Object entity : retList) {
					...
					if (idMap.containsKey(entity)) {
						MethodUtil.doSetMethod(entity, "Id", String.class,
								idMap.get(entity));
					} else if (idMap.containsKey(getObjKey(entity))) {
						MethodUtil.doSetMethod(entity, "Id", String.class,
								idMap.get(getObjKey(entity)));
					}
				}
			}
			return retObj;
	...
	private String getObjKey(Object entity) throws IllegalAccessException,
	...
	public ActionLogUtil getActLogUtil() {
		return actLogUtil;
	}

	@Autowired
	public void setActLogUtil(ActionLogUtil actLogUtil) {
		this.actLogUtil = actLogUtil;
	}
